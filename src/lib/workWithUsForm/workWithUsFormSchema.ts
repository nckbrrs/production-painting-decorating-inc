...
export const workWithUsFormSchema = z.object({
  ...
  experienceType: z.string({ error: "Please select your experience type" }),
  specialSkills: z
    .string()
    .max(500, { message: "Special skills must not exceed 500 characters" })
    .optional()
    .or(z.literal("")),
  ...
  desiredPay: z.string().min(1, { message: "Please enter your desired pay" }),
  payType: z.enum(["hourly", "yearly"], { error: "Please select pay type" }),
  preferredLanguage: z.string().min(1, { message: "Please enter your preferred language" }),
});
